...
async def test_upload_profile_photo(db_session, user):
    ...
    user: User = await UserService.get_by_email(db_session, user.email)
    assert user.profile_picture_url is not None, "The profile photo should be uploaded"
    profile_picture_url = f"/uploads/{user.nickname}/image.jpeg"
    assert (
